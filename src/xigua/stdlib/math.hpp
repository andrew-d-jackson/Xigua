...
class less_than : public comparator {
  bool compare(data a, data b) const { return (a.as_number() < b.as_number()); }
};
...
class equal_to : public comparator {
  ...
  bool compare(data a, data b) const { return (a == b); }
};
...
class add : public math_operation {
  long double operate(long double a, long double b) const { return a + b; }
};
...
class modulo : public method {
  ...
  data run(std::vector<data> args, enviroment &env,
    ...
    return make_number(
        std::fmod(args.at(0).as_number(), args.at(1).as_number()));
  }
};
}
}