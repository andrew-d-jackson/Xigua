...
class concatinate : public method {
  ...
  data_ptr run(call_info fci) {
    ...
    return make_string(return_string);
  }
};
...
class substring : public method {
  ...
  data_ptr run(call_info fci) {
    ...
    std::string start_string = fci.args.at(0)->as_string().as_std_string();
    int start_pos = (int)fci.args.at(1)->as_integer().as_int();
    int length = (int)fci.args.at(2)->as_integer().as_int() - start_pos;
