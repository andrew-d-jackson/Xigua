	...
	data at(std::vector<data> inputs, enviroment* execution_enviroment, std::vector<std::string> function_call_list)
	{
		if (inputs.at(0).type() == data_type::Number && inputs.at(1).type() == data_type::Tuple) {
			if (inputs.at(0).number() < 0  || inputs.at(0).number() > inputs.at(1).tuple().size() - 1)
				throw xig::error(xig::error_types::invalid_arguments, "Not In Range Of Tuple", function_call_list);
			...
			if (location != map.end()) {
				...
				throw xig::error(xig::error_types::invalid_arguments, "Not In HashMap", function_call_list);
			}
		} else {
			throw xig::error(xig::error_types::invalid_arguments, "Not A HashMap Or A Tuple", function_call_list);
		}
