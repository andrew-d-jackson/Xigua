...
TEST(Standard_Library_Core, Define) {
  ...
  EXPECT_EQ(evaluate(env, parser::from_string("[i]")), make_number(2));
...
TEST(Standard_Library_Core, Overload) {
  ...
  EXPECT_EQ(evaluate(env, parser::from_string("[f2 1 1]")), make_number(4));

  EXPECT_EQ(evaluate(env, parser::from_string("[f2 1]")), make_number(6));
}
